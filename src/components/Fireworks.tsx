...
const Fireworks: React.FC<FireworksProps> = ({ density = 0.05, autoLaunch = true, remainingTime, }) => {
  ...
  const colors = [
    "#FF0000", "#008000", "#FFFFFF", "#FFD700", "#00BFFF", "#FF69B4", "#FF8C00", "#ADFF2F"
  ];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      ctxRef.current.fillText("Merry Christmas", canvasRef.current.width / 2, canvasRef.current.height / 2);

      ctxRef.current.font = `bold ${responsiveSubFont()}px 'Mountains of Christmas', serif`;
      ctxRef.current.fillText('"Hope your christmas is merry, bright and full of love"', canvasRef.current.width / 2, canvasRef.current.height / 2 + responsiveTextLine());
  ...
  <>
    <div id="fireworks" className="w-full h-full fixed hidden opacity-0 z-40">
      <canvas ref={canvasRef} onClick={handleClick} aria-label="Christmas Fireworks canvas" id="fireworks" className="fixed top-0 left-0 w-full h-full" />;
