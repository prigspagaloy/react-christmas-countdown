...
export default function Countdown() {
    ...
    let targetDate = new Date(`December 25, ${currentYear}`).getTime();
    ...
    useEffect(() => {
        if (currentTime > targetDate) {
            targetDate = new Date(`December 25, ${setCurrentYear(prev => prev + 1)}`).getTime();
        }
    }, [remainingTime]);
    ...
    return (
        ...
        <div className="p-[1rem] relative font-christmas font-bold text-white text-shadow-[2px_2px_1px_#000000] flex flex-col gap-10 z-10">
            <h1 className="text-6xl md:text-8xl text-center self-center">Christmas Countdown {currentYear}</h1>
            <div>
                <div className="flex flex-col lg:flex-row lg:flex-wrap items-center-safe justify-center-safe gap-3 md:gap-5 lg:gap-7 text-4xl md:text-6xl text-center">
                    ...
                    <h3 className="md:w-full text-4xl md:text-6xl text-center">until Christmas</h3>
                </div>
            </div>
        </div>
        </>
